...
def _variable_with_weight_decay(name, shape, stddev, wd, use_xavier=True):
  ...
  Args:
    ...
    wd: add L1Loss weight decay multiplied by this float. If None, weight
  ...
  if wd is not None:
    weight_decay = tf.multiply(tf.reduce_sum(tf.abs(var)), wd, name='weight_loss')
...
def batch_norm_template(inputs, is_training, scope, moments_dims_unused, bn_decay, data_format='NHWC'):
  ...
  return tf.contrib.layers.batch_norm(inputs, decay=bn_decay,
                                      center=True, scale=False,
                                      is_training=is_training,
                                      scope=scope, zero_debias_moving_mean=False,
                                      data_format=data_format)
...
def dropout(inputs,
  ...
  with tf.variable_scope(scope) as sc:
    outputs = tf.cond(is_training,
                      lambda: tf.nn.dropout(inputs, rate=rate, noise_shape=noise_shape),
                      lambda: inputs)
    return outputs