...
def pointnet_sa_module_msg(xyz, points, npoint, radius_list, nsample_list, mlp_list, is_training, bn_decay, dropout_rate, scope, bn=True, use_xyz=True, use_nchw=False):
    """ 
    ...
    with tf.variable_scope(scope) as sc:
        ...
        for i in range(len(radius_list)):
            ...
            for j,num_out_channel in enumerate(mlp_list[i]):
                ...
                grouped_points = tf_util.dropout(grouped_points, is_training, scope='drpout%d_%d'%(i, j), rate=dropout_rate)
...
def pointnet_fp_module(xyz1, xyz2, points1, points2, mlp, is_training, bn_decay, dropout_rate, scope, bn=True):
    """
    ...
    with tf.variable_scope(scope) as sc:
        ...
        for i, num_out_channel in enumerate(mlp):
            ...
            new_points1 = tf_util.dropout(new_points1, is_training, scope='drpout_%d'%(i), rate=dropout_rate)
